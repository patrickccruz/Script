...
if (isset($_SESSION['user'])) {
    $user = $_SESSION['user'];
    echo "<!-- Debug: Usuário da sessão encontrado -->";
    echo "<!-- Debug: Nome do usuário = " . $user['name'] . " -->";
    echo "<!-- Debug: ID do usuário = " . $user['id'] . " -->";
} else {
    echo "<!-- Debug: Usuário não encontrado na sessão -->";
...
echo "<!-- Debug: Número de registros encontrados = " . $result->num_rows . " -->";

// Debug da consulta
if ($result->num_rows == 0) {
    echo "<!-- Debug: Nenhum registro encontrado para o usuário " . $user['id'] . " -->";
    // Vamos fazer uma consulta para ver todos os registros da tabela
    $debug_query = $conn->query("SELECT COUNT(*) as total, GROUP_CONCAT(DISTINCT user_id) as users FROM reports");
    $debug_info = $debug_query->fetch_assoc();
    echo "<!-- Debug: Total de registros na tabela = " . $debug_info['total'] . " -->";
    echo "<!-- Debug: IDs de usuários existentes = " . $debug_info['users'] . " -->";
}
?>
